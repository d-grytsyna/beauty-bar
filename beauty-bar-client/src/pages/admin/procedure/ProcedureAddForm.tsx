...
interface ProcedureAddFormProps {
  onCancel: () => void;
}
const ProcedureAddForm: React.FC<ProcedureAddFormProps> = ({ onCancel }) => {
    ...
    switch (name) {
      case "name":
        if (
          ...
          setError(
            "Name must be more than 2 characters, up to 50 and contain only letters."
          );
          return false;
        } else setError("");
        break;
      ...
      case "price":
        ...
        if (
          ...
          setError(
            "Price must be a valid number, more than 10 and less than 1000."
          );
          return false;
        } else {
          setError("");
      ...
      case "time":
        if (value < "00:15" || value > "06:00") {
          setError(
            "Procedure duration must be between 15 minutes and 6 hours."
          );
          return false;
        } else {
          setError("");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isValid) {
      try {
        if (formData) await ProcedureService.addProcedure(formData);
        alert("Procedure successfully added!");
        onCancel();
      } catch (apiError: any) {
